...
function setupEventListeners() {
  ...
  webcamCheck.addEventListener('change', (e) => {
    ...
    if (e.target.checked && webcamSelect.options.length === 1) {
      enumerateDevices();
    }
  });
  ...
  microphoneCheck.addEventListener('change', (e) => {
    ...
    if (e.target.checked && micSelect.options.length === 1) {
      enumerateDevices();
    }
  });
}
...
async function enumerateDevices() {
  try {
    ...
    videoDevices.forEach(device => {
      ...
      option.text = device.label || `Webcam ${webcamSelect.options.length}`;
    ...
    updateStatus('⚠️ Impossible de lister les périphériques', true);
  }
}
